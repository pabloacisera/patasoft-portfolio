...
# Leer el hostname del tunnel desde el .env (lo seteás una sola vez)
...
if [ -z "$TUNNEL_HOSTNAME" ]; then
    ...
    warn "Continuando sin actualizar GOOGLE_CALLBACK_URL..."
else
    # Asegurar HTTPS
    ...
    # Actualizar GOOGLE_CALLBACK_URL en .env
    if grep -q "^GOOGLE_CALLBACK_URL=" "$BACKEND_ENV"; then
        sed -i "s|^GOOGLE_CALLBACK_URL=.*|GOOGLE_CALLBACK_URL=${CALLBACK_URL}|" "$BACKEND_ENV"
    else
        echo "GOOGLE_CALLBACK_URL=${CALLBACK_URL}" >> "$BACKEND_ENV"
    fi

    ok "Cloudflare Tunnel activo (PID: $TUNNEL_PID)"
    ok "Tunnel hostname: $TUNNEL_HOSTNAME"
    ok "GOOGLE_CALLBACK_URL → $CALLBACK_URL"
fi
...
#    Ahora lee el .env con GOOGLE_CALLBACK_URL correcto
# ─────────────────────────────────────────
